...
\end{lstlisting}
\end{minipage}
\captionof{lstlisting}{Type Declarations in \IOTDSL: Devices declare capabilities (i.e. sensing or actuating) as high-level events to be manipulated by end-users.}
...
\begin{lstlisting}[label=fig:RE-Network,language=iotdsl,caption=Network Configuration for the devices present in Alice's House]	
...
\end{lstlisting}
	...
	\item There is a critical fire situation when smoke is detected in the kitchen while temperature is maintained beyong 40°C for at least five minutes.
\end{enumerate}
